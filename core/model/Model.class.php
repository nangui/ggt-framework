...
class Model extends Param {
    ...
    public function getDatas($champ = array(), $order_by = 'id', $debut = 0, $limit = "") {
    ...
    public function getDatasNoLimit($champ = array(), $order_by = 'id', $clause = '') {
    ...
    public function getMoreById($id, $champ = array(), $track = false) {
    ...
    public function getIdByArray($champs = array(), $track = false) {
    ...
    public function recordExists($champs = array(), $track = false) {
    ...
    public function getIdsByArray($champs = array(), $clause = ' 1=1 ', $order = 'id', $track = false) {
    ...
    public function deleteByIdIn($id_data = array(), $clause = "", $track = false) {
    ...
    public function deleteByIdNIn($id_data = array(), $clause = "", $track = false) {
    ...
    public function insertOne($donnees = array(), $track = false) {
    ...
    public function updateOne($donnees = array(), $track = false) {
    ...
    public function updateWithFreeClause($donnees = array(), $clause = "", $track = false) {
        if (count($donnees) > 0) {
            $set = "";
    ...
    public function deleteWithFreeClause($tableau = array(), $track = false) {
        if (count($tableau) > 0) {
            $retour = 0;
            $tableau_requete = [];
    ...
    public function getResetInformationsByKeychain($clechain = array(), $etat = -2) {
        ...
        return ($reset_etat) ? array($reset_etat, $reset_message, $lignes) : array($reset_etat, $reset_message);
    }

    public function activateAccountByKeychain($clechain = array()) {
        ...
        if ($execution_requete->rowCount() > 0) {
            ...
            $this->updateOne(array('id' => $lignes->id, 'actif' => 1));
        ...
        return ($activation_etat) ? array($activation_etat, $activation_message, $lignes) : array($activation_etat, $activation_message);
    }
    ...
    public function uploadPicture($photo, $nom_enregistre, $chemin, $dimension = []) {
        ...
        if (isset($photo["name"]) && (!empty($photo["name"]))) {
            ...
            if (filesize($photo['tmp_name']) > _TAILLE_MAX_IMAGE) {
                return array(false, " Erreur lors du chargement; la taille de l'image est trop grande. (Lim. " . round(_TAILLE_MAX_IMAGE / 1000000, 0) . "Mo)");
            }
            // On vérifie l'extention.
    ...
    public function autocomplete($champ, $term) {
        ...
        $execution_requete = $this->dao->prepare($requete);
        $execution_requete->execute(array('term' => '%' . $term . '%'));

        $tableau = array(); // on créé le tableau
...
#[ - Fonctions spécifiques pour SLM - ]
    ...
    public function getElementType($types = array(), $pour = '') {
