...
abstract class Smarty_Internal_TemplateCompilerBase
{
     ...
     * @var array
     */
    public $plugin_search_order = ['function',
                                        'block',
                                        'compiler',
                                        'class', ];
    /**
    ...
    public function getPluginFromDefaultHandler($tag, $plugin_type)
    {
        ...
        if ($result) {
            ...
            if (is_callable($callback)) {
                $this->default_handler_plugins[$plugin_type][$tag] = [$callback,
                                                                               true,
                                                                               [], ];
    ...
    {
        return str_replace(['SMARTYldel', 'SMARTYliteral', 'SMARTYrdel', 'SMARTYautoliteral', 'SMARTYal'],
    ...
    private function compileTag2($tag, $args, $parameter)
    {
        ...
        if (isset($this->smarty->_cache['get_used_tags'])) {
            $this->template->_cache['used_tags'][] = [$tag,
                                                             $args, ];
        }
        ...
        if ($_output !== false) {
            ...
            if (strlen($tag) < 6 || substr($tag, -5) !== 'close') {
                ...
                foreach ([Smarty::PLUGIN_COMPILER,
